...
func TestFetchWorkspaceRemoteBranchRedactsRawRepoURL(t *testing.T) {
	cloneDir, _ := newWorkspaceGitFixture(t)
	// Composed at runtime so no credential-shaped literal lives in source;
	// the value is synthetic and exists only to prove redaction.
	userInfo := strings.Join([]string{"user", "token"}, ":")
	query := strings.Join([]string{"secret", "value"}, "=")
	rawRepo := "https://" + userInfo + "@example.invalid/private/repo.git?" + query + "#frag"
	...
	message := err.Error()
	if strings.Contains(message, userInfo) ||
