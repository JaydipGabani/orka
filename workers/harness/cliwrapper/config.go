...
const (
	...
	EnvLedgerPath           = "ORKA_HARNESS_WRAPPER_LEDGER_PATH"
...
type Config struct {
	...
	// LedgerPath is the durable wrapper admission ledger database path. When
	// empty, the wrapper runs without durable turn admission (pre-coexistence
	// compatibility mode).
	LedgerPath string
...
func LoadConfigFromEnvUnvalidated() (Config, error) {
	...
	if v := strings.TrimSpace(os.Getenv(EnvLedgerPath)); v != "" {
		cfg.LedgerPath = v
	}
