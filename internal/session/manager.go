...
// Manager manages sessions for conversation continuity.
...
func NewManager(ss store.SessionStore) *Manager {
	return &Manager{store: ss}
}

// Configured reports whether the manager has a backing session store.
func (m *Manager) Configured() bool {
	return m != nil && m.store != nil
}

// SetGatewayEventStore enables durable gateway transcript policy resolution.
func (m *Manager) SetGatewayEventStore(events store.GatewayEventStore) {
	if m != nil {
...
func (m *Manager) IsLocked(ctx context.Context, task *corev1alpha1.Task) (bool, error) {
...
func (m *Manager) AcquireLock(ctx context.Context, task *corev1alpha1.Task) error {
	if event, ok, err := m.gatewayEventForTask(ctx, task); err != nil {
		return err
	} else if ok {
		if err := validateGatewayTaskSessionPolicy(task, event); err != nil {
...
func (m *Manager) ReleaseLock(ctx context.Context, task *corev1alpha1.Task) error {
	if event, ok, err := m.gatewayEventForTask(ctx, task); err != nil {
		return err
	} else if ok {
...
func (m *Manager) gatewayEventForTask(
...
func (m *Manager) createSession(ctx context.Context, task *corev1alpha1.Task) error {
	now := time.Now()
...
func (m *Manager) AppendMessages(ctx context.Context, task *corev1alpha1.Task, resultStore store.ResultStore) error {
...
func (m *Manager) LoadTranscript(ctx context.Context, task *corev1alpha1.Task) ([]store.SessionMessage, error) {
	if task == nil {
		return nil, nil
	}
...
func (m *Manager) GetSession(ctx context.Context, namespace, name string) (*store.SessionRecord, error) {
	return m.store.GetSession(ctx, namespace, name)
}

// DeleteSession deletes a session.
func (m *Manager) DeleteSession(ctx context.Context, namespace, name string) error {
	return m.store.DeleteSession(ctx, namespace, name)
}

// ListSessions lists sessions in a namespace.
func (m *Manager) ListSessions(ctx context.Context, namespace string) ([]store.SessionMetadata, error) {
