...
- RuntimePools are controller-owned, digest-pinned, scale-to-zero resources. Only `Serving` + `Accepting` admits new RuntimeSessions; drain/finalization must complete before replacement or scale-down. A timed-out rollout drain preserves the old instance only while prompt work is in flight and recycles it once only stranded sessions remain; `status.activeInstance.runtimePoolGeneration` records the admitted generation so the planned upgrade drain can validate a preserved pre-rollout instance; a pool whose image is no longer controller-approved drains its old workload and converges to `Stopped`/`RolloutFailed` instead of parking `Degraded`.
